...
pub struct GameState {
    ...
    /// GPU 描画スレッドが書き込む FPS 値（Mistral から engine.fps() で取得）
...
impl Interpreter {
    ...
    fn call_engine(&mut self, func: &str, args: &[Expr]) -> Result<Value, String> {
        match func {
            "fps"    => Ok(Value::Int(self.state.fps.load(Ordering::Relaxed) as i64)),
            "width"  => Ok(Value::Int(self.state.screen_w.load(Ordering::Relaxed) as i64)),
            "height" => Ok(Value::Int(self.state.screen_h.load(Ordering::Relaxed) as i64)),
